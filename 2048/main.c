...
#define LSM303AGR_ADDR_WRITE 0x32  // 0x19 << 1, for the MSA and setting bit 0 to 0 for write
#define LSM303AGR_ADDR_READ  0x33  // (0x19 << 1) | 1, for the MSA and setting bit 0 to 1 for read
...
void i2c_init() {
    ...
    *GPIO_DEN(gpio_port_a) |= i2c_pins;       // Digital enable
    ...
    // Set port control for I2C1 (function 3)
    *GPIO_PCTL(gpio_port_a) &= ~0xFF000000;   // Clear PMC6 and PMC7
    *GPIO_PCTL(gpio_port_a) |= 0x33000000;    // Set PMC6=3, PMC7=3
    
    // Configure I2C1 master
    *I2C_MCR(I2C_1) = 0x10;                   // Enable master
    *I2C_MTPR(I2C_1) = 7;                     // 100kHz clock
}
